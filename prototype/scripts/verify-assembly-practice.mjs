...
export async function verifyAssemblyPractice(page,artifactDir){
  ...
  const workshop=page.getByRole('region',{name:'3D 交互装机工作台'}),review=page.getByRole('region',{name:'练习复盘'});
  ...
  await expect(review.locator('.practice-metrics')).toContainText('95');
  ...
  console.log('  PASS guided, independent, all fault repairs, review, restart, mobile and order isolation');
}
