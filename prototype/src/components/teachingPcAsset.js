...
export async function loadTeachingAsset() {
 if(!bytes) bytes=fetch(import.meta.env.BASE_URL+'models/teaching-pc.glb',{signal:AbortSignal.timeout(10000)})
 .then(r=>{if(!r.ok)throw new Error('模型请求失败 '+r.status);return r.arrayBuffer();})
 .then(buffer=>{validateTeachingGlb(buffer);return buffer;}).catch(error=>{bytes=null;throw error;});
 ...
 if(size.x<.3 || size.x>.6 || size.y<.15 || size.z>.6){disposeTeachingAsset(asset);throw new Error('模型尺寸不符合教学主机规范');}
