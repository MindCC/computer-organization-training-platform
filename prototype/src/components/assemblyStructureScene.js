...
export function createStructureScene(scene,partGroups,asset,registry) {
 ...
 for(const name of ['side_panel','cooler']){
  ...
  if(node){node.removeFromParent();node.position.multiplyScalar(3.1);node.scale.multiplyScalar(3.1);}
  else {
 ...
 const wires=Object.entries(paths).map(([id,points],index)=>{
 ...
 return {update(state,reducedMotion){
  ...
  extras.get('cooler').visible=Boolean(s.cooler);
