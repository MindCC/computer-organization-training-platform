...
export function CpuExecutionPanel() {
  ...
  function apply(result) { setMachine(result.state); setMessage(result.trace?.explanation ?? (result.state.trap || (result.state.halted ? "程序已停止。" : "程序执行完成。"))); }
  function loadSource() { try { setMachine(createEdu16State({ program: assembleEdu16(source), memory: DEFAULT_MEMORY })); setMessage("程序已载入。请用微步观察取指、译码和执行。"); } catch (error) { setMessage(error.message); } }
  function run() { const result = runEdu16(machine); setMachine(result.state); setMessage(result.state.trap || `已执行 ${result.instructionsExecuted} 条指令。`); }
  return <section className="cpu-execution-panel" aria-label="CPU 单步执行实验">
    ...
    <div className="cpu-execution-grid">
      <label className="cpu-program"><span>教学程序</span><textarea aria-label="EDU16 教学程序" onChange={(event) => setSource(event.target.value)} spellCheck="false" value={source} /><button className="ghost-button" onClick={loadSource} type="button">载入程序</button></label>
      <section className="cpu-datapath"><span className="eyebrow">当前微步骤</span><strong>{phaseName(machine.phase)}</strong><div className="cpu-transfer-row">{transfer(machine.phase).map((item) => <code key={item}>{item}</code>)}</div><p>{message}</p><div className="cpu-controls"><button className="primary-button" disabled={Boolean(machine.halted || machine.trap)} onClick={() => apply(stepMicro(machine))} type="button"><SkipForward size={17} />微步</button><button className="ghost-button" disabled={Boolean(machine.halted || machine.trap)} onClick={() => apply(executeInstruction(machine))} type="button"><FastForward size={17} />执行一条</button><button className="ghost-button" disabled={Boolean(machine.halted || machine.trap)} onClick={run} type="button"><Play size={17} />运行</button><button className="ghost-button" onClick={() => { setMachine(initialMachine()); setSource(DEFAULT_SOURCE); setMessage("已恢复 6 + 1 的教学预设。"); }} type="button"><ArrowCounterClockwise size={17} />重置</button></div></section>
