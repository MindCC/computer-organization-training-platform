...
- **3D 包优化仍未落地**：产物 `OverviewExplodedView` 为 1,004.19 kB，gzip 277.42 kB，仍有大包提示。已有原生 Three.js 迁移设计/计划，不能将“已写方案”等同于“已完成迁移”。先保留性能基线，再按方案实施。
...
- Windows 在收尾阶段开始拒绝创建 Node/esbuild 子进程（`spawn EPERM`），因此新的 `npm test`、构建和浏览器专项无法在本机再次启动；这不是断言失败。变更前本轮的 3D 专项 29/29、3D 性能专项和生产构建均通过，待该环境恢复后应重跑完整门禁。
