...
已修复课程工作台两处身份覆盖：建立小组只采用已认证教师和 URL 草稿 ID，学生成果提交只采用已认证学生及 URL 中的项目和里程碑。路由集成测试覆盖跨教师建组与跨学生提交的伪造请求，均应返回 404。

学生再次编辑未评价成果时，界面会为该编辑版本建立新的 `clientSubmissionId`；同一在途草稿保留该标识，以继续支持网络重试。教师工作台现读取待评价成果，展示项目、小组、学生、角色、反思和证据链接，并通过既有受权接口提交评价。

课堂浏览器脚本改用当前登录表单的可访问名称和提交按钮，并由 `qa:classroom` 先构建生产包后执行。`npm audit --json` 在本轮环境返回 0 个漏洞，因此未进行没有安全收益的依赖升级。

本次仍不包含真实教室的集显、8GB 内存、网络与多机长时试运行；完成这些现场验收前，不应将本地或无头浏览器结果表述为课堂硬件兼容性保证。

本轮复验结果：`npm test` 334/334 通过；生产构建通过；生产双上下文课堂流程通过且无页面错误；150 学生、30 并发课堂负载门禁 P95 为 38ms、无失败、无 `SQLITE_BUSY`；构建预算通过。
