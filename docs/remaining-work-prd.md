...
日期：2026-07-20
基线：HEAD `95de857` + 3D 修复（2026-07-20 第二轮）
关联文档：
...
本文档汇总平台当前所有未完成或未闭环的需求，作为下一阶段产品交付与工程实施的依据。接手者阅读本文档即可理解待做项的背景、范围、验收标准和优先级，无需翻阅完整聊天记录。
...
剩余待做项集中在：教师学生详情分析收尾、学生复盘闭环（错题本）、运维可观测性（audit_logs）、若干 P1/P2 收尾项，以及测试环境修复。
